...
int within_eps(float a, float b) { return a - EPS < b && b < a + EPS; }
...
int same_image(image a, image b) {
  ...
  for (i = 0; i < a.w * a.h * a.c; ++i) {
    if (!within_eps(a.data[i], b.data[i])) {
      printf("The value at %d should be %f, but it is %f! \n", i, b.data[i],
             a.data[i]);
      return 0;
    }
  }
...
void test_sobel() {
  ...
  //   printf("gt_mag: width = %d, height = %d, channels = %d\n", gt_mag.w,
  //   gt_mag.h,
  //          gt_mag.c);
  //   printf("mag: width = %d, height = %d, channels = %d\n", mag.w, mag.h,
  //   mag.c);

  if (gt_mag.w != mag.w || gt_theta.w != theta.w || gt_mag.h != mag.h ||
      gt_theta.h != theta.h || gt_mag.c != mag.c || gt_theta.c != theta.c)
    return;
  int i;
